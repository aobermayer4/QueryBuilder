...
db_rules <- list(
  pubmed = list(
    name="PubMed",
    mesh_fmt  = function(t)       paste0('"',t,'"[MeSH]'),
    exp_fmt   = function(t,)      paste0('"',t,'"[MeSH]'),
    tiab_fmt  = function(t,p=NA)  if(!is.na(p)) paste0('"',gsub("\\*","",t),'"[tiab:~',p,']') else paste0('"',t,'"[tiab]'),
  ...
  wos = list(
    ...
    open="(", close=")", and=" AND ", or=" OR ", field="", wrap=function(s) paste0("( ", s, " )")
  ),
  embase = list(
    name="Embase",
    exp_fmt   = function(t)       paste0("'",t,"'/exp"),
    tiab_fmt  = function(t,p=NA)  if(!is.na(p)) paste0("'",t,"':ti,ab,kw") else paste0("'",t,"':ti,ab,kw"),
    mesh_fmt  = function(t,p=NA)  if(!is.na(p)) paste0("'",t,"':ti,ab,kw") else paste0("'",t,"':ti,ab,kw"),
    plain_fmt = function(t)       if(grepl(" ", t)) paste0('"',t,'"') else t,
    open="(", close=")", and=" AND ", or=" OR ", wrap=function(s) paste0("( ", s, " )")
  ),
  scopus = list(
...
format_term <- function(tm, db) {
  ...
  if (db=="wos")    return(near_wrap(term,"NEAR/"))
  if (db=="scopus") return(near_wrap(term,"W/"))
  parts <- character(0)
  if (db=="embase") {
    if ("tiab"  %in% flags) {
    ...
    #if (type=="mesh") return(r$mesh_fmt(term))
    #if (!is.na(prox) && grepl(" ",term)) {
    #  w <- strsplit(term," ")[[1]]
    #  return(paste0("'",paste(w,collapse=paste0(" near/",prox," ")),"':ti,ab,kw"))
    #}
    #if (type=="tiab") return(r$tiab_fmt(term,prox))
    #return(r$plain_fmt(term))
  }
  if (db=="pubmed") {
...
build_query <- function(concepts, db) {
  ...
  strs <- vapply(cons, function(con) {
    ...
    else if (db=="wos") paste0("TS=",r$open,inner,r$close)
...
.app-header-sub{font-size:1.2rem;color:var(--tx-sub);font-family:'IBM Plex Mono',monospace;}
.io-toolbar{display:flex;align-items:center;gap:7px;}
.io-btn{background:var(--io-bg);border:1px solid var(--io-bd);color:var(--tx-head);border-radius:6px;padding:6px 14px;font-size:1.3rem;cursor:pointer;font-family:'IBM Plex Sans',sans-serif;transition:all .15s;display:flex;align-items:center;gap:5px;}
...
.panel-header{padding:10px 13px 8px;font-size:1rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--tx-lbl);border-bottom:1px solid var(--bd-base);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
...
.concept-card-label{flex:1;font-size:1.2rem;font-weight:600;color:var(--tx-sec);min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.concept-card-count{font-size:1rem;color:var(--tx-cnt);font-family:'IBM Plex Mono',monospace;background:var(--bg-input);border-radius:10px;padding:2px 7px;flex-shrink:0;}
...
.concept-card-preview{padding:0 10px 8px;font-size:1rem;color:var(--tx-prev);font-family:'IBM Plex Mono',monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer;}
.btn-del-concept{background:none;border:none;color:var(--delc-tx);cursor:pointer;font-size:1.3rem;padding:1px 4px;border-radius:4px;transition:color .15s;flex-shrink:0;}
.btn-del-concept:hover{color:var(--del-hov);}
.btn-add-concept{margin:5px 7px 7px;background:transparent;border:1px dashed var(--addc-bd);color:var(--addc-tx);border-radius:7px;padding:8px;width:calc(100% - 14px);font-size:1.2rem;cursor:pointer;transition:all .15s;}
...
.concept-title-input{background:transparent;border:none;border-bottom:2px solid var(--bd-base);color:var(--tx-sec);font-size:2.15rem;font-weight:600;font-family:'IBM Plex Sans',sans-serif;outline:none;flex:1;padding-bottom:4px;transition:border-color .15s;}
.concept-title-input:focus{border-color:var(--bd-foc);}
.include-badge{font-size:1rem;font-family:'IBM Plex Mono',monospace;border-radius:5px;padding:3px 8px;flex-shrink:0;}
...
/* term text — plain display, click to edit */
.term-text-display{
  font-family:'IBM Plex Mono',monospace;font-size:1.3rem;color:var(--tx-mono);
  padding:4px 6px;border-radius:4px;cursor:text;min-width:0;word-break:break-all;
  border:1px solid transparent;transition:border-color .15s;
}
.term-text-display:hover{border-color:var(--bd-input);}
.term-text-edit{
  font-family:'IBM Plex Mono',monospace;font-size:1.3rem;color:var(--tx-mono);
  background:var(--bg-input);border:1px solid var(--bd-foc);border-radius:4px;
  padding:4px 6px;outline:none;width:100%;
}

/* Multi-flag chips */
.type-chips{display:flex;gap:3px;flex-shrink:0;}
.flag-chip{font-family:'IBM Plex Mono',monospace;font-size:1rem;border-radius:10px;padding:2px 8px;cursor:pointer;border:1px solid;white-space:nowrap;transition:all .15s;opacity:.45;}
...
.prox-badge{font-family:'IBM Plex Mono',monospace;font-size:qrem;background:var(--prox-bg);color:var(--prox-tx);border:1px solid var(--prox-bd);border-radius:10px;padding:2px 8px;cursor:pointer;white-space:nowrap;}
...
.prox-input-inline{font-family:'IBM Plex Mono',monospace;font-size:1rem;width:46px;text-align:center;background:var(--bg-input);border:1px solid var(--prox-bd);color:var(--prox-tx);border-radius:6px;padding:2px 4px;outline:none;}


.btn-del{background:none;border:none;color:var(--del-tx);cursor:pointer;font-size:1rem;padding:2px 6px;border-radius:4px;transition:color .15s;flex-shrink:0;}
.btn-del:hover{color:var(--del-hov);}

/* Add term row */
.add-term-row{display:flex;gap:7px;align-items:center;margin-top:8px;flex-shrink:0;}
.term-new-input{flex:1;background:var(--bg-input);border:1px solid var(--bd-input);color:var(--tx-mono);border-radius:6px;padding:6px 10px;font-size:1.3rem;font-family:'IBM Plex Mono',monospace;outline:none;}
.term-new-input:focus{border-color:var(--bd-foc);}
.btn-add-term{background:var(--btn-bg);border:1px solid var(--btn-bd);color:var(--btn-tx);border-radius:6px;padding:6px 14px;font-size:1.2rem;cursor:pointer;transition:all .15s;}
...
.db-tab{background:var(--tab-bg);border:1px solid var(--bd-base);color:var(--tab-tx);border-radius:5px;padding:4px 11px;font-size:1.2rem;cursor:pointer;font-family:'IBM Plex Mono',monospace;transition:all .15s;}
...
.included-summary{font-size:1.2rem;color:var(--sum-tx);font-family:'IBM Plex Mono',monospace;}
.included-summary span{color:var(--sum-hl);}
.query-box{background:var(--bg-output);border:1px solid var(--bd-output);border-radius:7px;padding:10px 12px;font-family:'IBM Plex Mono',monospace;font-size:1.2rem;color:var(--tx-query);white-space:pre-wrap;word-break:break-all;max-height:115px;overflow-y:auto;line-height:1.6;}
.copy-btn{margin-top:7px;background:var(--copy-bg);border:1px solid var(--copy-bd);color:var(--copy-tx);border-radius:6px;padding:5px 16px;font-size:1.2rem;cursor:pointer;transition:all .15s;float:right;}
...
.empty-state{display:flex;flex-direction:column;align-items:center;justify-content:center;flex:1;color:var(--tx-faint);font-size:1.3rem;gap:8px;padding:30px;text-align:center;}
...
// ── state ──────────────────────────────────────────────────────────────────
var terms = [];       // [{text, type, prox}]  — JS-side ground truth
var conceptIdx = -1;  // 1-based concept index currently shown

// ── push current terms to Shiny ───────────────────────────────────────────
function pushTerms() {
...
function proxLabel(p) {
  return (p===null||p===undefined||p==='') ? null : 'NEAR/' + p;
}
...
function renderAll() {
  ...
  // update card preview in left panel
  var prev = terms.slice(0,2).map(function(t){ return t.text; }).join(' OR ');
  var lbl = document.getElementById('card_label_' + conceptIdx);
  if (lbl) {
...
function makeRow(tm, i) {
  ...
  // 1. Term text — click to edit inline
  ...
  // 2. Type chip — click cycles tiab → mesh → plain → tiab
  var chip = document.createElement('button');
  chip.className = 'type-chip';
  chip.dataset.type = tm.type;
  chip.textContent = typeLabel(tm.type);
  chip.title = 'Click to cycle type: tiab → MeSH/Exp → Plain';
  chip.addEventListener('click', function() {
    terms[i].type = nextType(terms[i].type);
    pushTerms();
    renderAll();
  });
  row.appendChild(chip);

  // 3. Prox badge — click to set/clear
  var proxWrap = document.createElement('div');
  proxWrap.className = 'prox-wrap';
  var proxEl = makeProxEl(i, tm.prox);
  proxWrap.appendChild(proxEl);
...
function makeProxEl(i, prox) {
  ...
  if (lbl) {
    // Show filled badge; click to clear or edit
    var badge = document.createElement('button');
    badge.className = 'prox-badge';
    badge.textContent = lbl;
    badge.title = 'Proximity — click to edit, double-click to clear';
    ...
    badge.textContent = '~/n';
    badge.title = 'Set proximity (NEAR/n) — click to add';
    badge.addEventListener('click', function() { startEditProx(i, badge); });
    return badge;
  }
}
...
function startEditText(i, display, wrap) {
  ...
  inp.type = 'text';
  inp.className = 'term-text-edit';
  inp.value = terms[i].text;
...
function startEditProx(i, badge) {
  ...
  inp.addEventListener('keydown', function(e) {
    if (e.key==='Enter') { e.preventDefault(); inp.blur(); }
    if (e.key==='Escape') { terms[i].prox = null; renderAll(); }
  });
}

// ── public: load a concept's terms ────────────────────────────────────────
window.termEditor = {
  load: function(ci, termData) {
    conceptIdx = ci;
    terms = (termData || []).map(function(t) {
      return { text: t.text, type: t.type || 'tiab', prox: t.prox || null };
    });
    renderAll();
  },
  addTerm: function(text) {
    if (!text.trim()) return;
    terms.push({ text: text.trim(), type: 'tiab', prox: null });
    pushTerms();
    renderAll();
  }
};
...
ui <- fluidPage(
  ...
  div(class="app-header",
      ...
      div(class="io-toolbar",
          ...
          tags$button(id="theme_toggle", class="theme-btn", title="Toggle light/dark mode", "🌙",
                      onclick="
        ...
        "),
          ...
          r.onload=function(e){ Shiny.setInputValue('load_file_data',{name:f.name,data:e.target.result.split(',')[1]},{priority:'event'}); };
      ...
      div(class="concepts-panel",
          div(class="panel-header",
              ...
              actionButton("add_concept","+ Add",class="btn-add-concept",
                           style="margin:0;padding:3px 10px;width:auto;font-size:1.2rem;")
          ),
      ...
      div(class="editor-panel",
          ...
          div(class="output-panel",
              ...
              tags$button("Copy", id="copy_btn", class="copy-btn",
                          onclick="
            var t=document.getElementById('query_display').innerText;
...
server <- function(input, output, session) {
  ...
  # terms_update: JS sends full term array back after any change -----------------------------------------------------
  # This replaces the per-field observer pattern entirely.
  observeEvent(input$terms_update, {
    ci    <- as.integer(input$terms_update$ci)
    tlist <- input$terms_update$terms   # list of lists from JSON
    cons  <- concepts()
    if (is.null(cons[[ci]])) return()
    cons[[ci]]$terms <- lapply(tlist, function(t) list(
      text = as.character(t$text %||% ""),
      type = as.character(t$type %||% "tiab"),
      prox = { v <- suppressWarnings(as.integer(t$prox)); if(is.na(v)||v<=0||!isTruthy(v)) NA_integer_ else v }
    ))
    concepts(cons)
  }, ignoreInit=TRUE)
  
  # Add term button: tell JS to add the term, then clear + refocus input -----------------------------------------------------
  observeEvent(input$add_term_btn, {
    txt <- trimws(input$new_term_text %||% "")
    if (!nzchar(txt)) return()
    runjs(sprintf("if(window.termEditor) window.termEditor.addTerm(%s);",
  ...
  observe({
    runjs("
      $(document).off('keydown.addterm').on('keydown.addterm','#new_term_text',function(e){
        if(e.key==='Enter'){ e.preventDefault(); Shiny.setInputValue('add_term_btn',Math.random(),{priority:'event'}); }
      });
    ")
  })
  ...
  output$save_rds <- downloadHandler(
    ...
    content =function(file) saveRDS(list(version="1.0",saved_at=Sys.time(),concepts=concepts()),file)
  )
  observeEvent(input$load_file_data, {
    tryCatch({
      ...
      cons <- lapply(sess$concepts,function(c){ if(is.null(c$included)) c$included<-TRUE; c })
  ...
  # Render editor skeleton — JS fills the term rows -----------------------------------------------------
  # Only re-renders on concept switch (active_concept changes) or structural
  # changes (concept added/deleted). Term edits go through JS → terms_update
  # → concepts(), but renderUI does NOT depend on concepts() directly; it only
  # depends on active_concept() and the name/included fields it needs for the
  # title row. This prevents the table from ever being torn down during editing.
  output$concept_editor <- renderUI({
    ac   <- active_concept()
    cons <- isolate(concepts())   # isolate: don't re-render on every term change
    ...
    # Serialize term data for JS
    term_json <- jsonlite::toJSON(
      lapply(con$terms, function(t) list(
        text = t$text,
        type = t$type,
        prox = if(is.na(t$prox)||is.null(t$prox)) NULL else t$prox
      )),
    ...
    tagList(
      ...
      # Instruction hint
      div(style="font-size:1rem;color:var(--tx-mut);margin-bottom:8px;flex-shrink:0;",
          "Add terms below, then click the ", tags$strong("type chip"), " to cycle tiab → MeSH/Exp → Plain, ",
          "and the ", tags$strong("~/n badge"), " to set proximity."
      ),
      
      # JS-owned term list — R renders this div empty; JS populates it
      ...
      # Boot the JS editor with current term data
  ...
  # Make concept_editor re-render when switching concepts (active_concept changes)
  # but NOT when only terms change inside the same concept.
  # We achieve this by having renderUI depend on active_concept() but isolate concepts().
  # When terms_update fires, concepts() updates but active_concept() does not change,
  # so renderUI does NOT re-run. When the user switches concept, active_concept() changes,
  # renderUI re-runs, fetches the fresh concepts() via isolate, and loads the new concept's terms.
